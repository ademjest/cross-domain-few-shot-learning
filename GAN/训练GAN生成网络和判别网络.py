...
# 数据目录
route = r'G:\Python\python_work\conda_test\metric_learning_few_shots\prototypical-networks-few-shot-learning-main' \
        r'\datasets\officehome'  # 数据目录
...
# 用于生成标签转换图label_map，为了在LoadDate中将字符型标签如“Alarm_clock”转为数字0
path = os.path.join(route, 'train')
data_cache = datasets.ImageFolder(path)
label_map = data_cache.class_to_idx
...
#     fake_img=i(fake_img)
categories = os.listdir(os.path.join(route, 'train'))
for number in categories:  # 0-9每一个数字单独训练
    print(number)
    # 初始化网络每一层的参数
    ...
    # 初始化训练数据读取器
    train_dataset = LoadData(os.path.join(route, 'train', number), number_of_channels=g_d_nc, category=number,
                             label_map=label_map)
