...
class OmniglotDataset(Dataset):
    ...
    def __getitem__(self, item):
        ...
        instance = instance.astype(np.float32)  # 将数据转换为浮点型数组
...
class OfficeHomeDataset(Dataset):
    def __init__(self, subset):
        """Dataset class representing OfficeHome dataset
        ...
        # 调试信息
        print(f"Indexing subset: {self.subset}")
        subset_data = self.index_subset(self.subset)
        print(f"Indexed {len(subset_data)} images")

        self.df = pd.DataFrame(subset_data)
        self.df = self.df.assign(id=self.df.index.values)

        self.unique_classes = sorted(self.df['class_name'].unique())
        self.class_name_to_id = {self.unique_classes[i]: i for i in range(self.num_classes())}
        self.df = self.df.assign(class_id=self.df['class_name'].apply(lambda c: self.class_name_to_id[c]))
    ...
    def index_subset(subset):
        ...
        print('Indexing {}...'.format(subset))
        subset_len = 0
        # 确认 DATA_PATH 指向 ./data，而 officehome 数据集在 ./data/officehome 目录下
        target_path = os.path.join(DATA_PATH, 'officehome', f'images_{subset}')
        print(f"Target path for {subset}: {target_path}")
        ...
        for root, folders, files in os.walk(target_path):
            ...
            print(f"Processing class: {class_name} in root: {root}")
